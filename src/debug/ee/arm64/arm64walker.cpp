...
void NativeWalker::Decode()
{
    ...
    m_nextIP = m_ip + MAX_INSTRUCTION_LENGTH;
    ...
    if (NativeWalker::DecodeCallInst(context, opcode, RegNum, m_type)) //Unconditional Branch (register) instructions
    {
...
BYTE*  NativeWalker::SetupOrSimulateInstructionForPatchSkip(T_CONTEXT * context, SharedPatchBypassBuffer* m_pSharedPatchBypassBuffer,  const BYTE *address, PRD_TYPE opcode)
{
    ...
    */
    ...
    if ((opcode & 0x1F000000) == 0x10000000)  //ADR & ADRP
    {
        ...
        TADDR immhigh = (opcode & 0x00FFFFE0) << 2;
    ...
    else if ((opcode & 0xFF000000) == 0x58000000) //Load register (literal)
    {
        
        offset = (opcode & 0x1FFFFFE0) << 2;
    ...
    else if (NativeWalker::DecodeCallInst(context, opcode, RegNum, walk)) 
    {
        _ASSERTE(offset == 0);
    }
    ...
    else if(RegNum >= 0)
    {
        CORDbgSetInstruction((CORDB_ADDRESS_TYPE *)patchBypass, 0xd503201f); //Add Nop in buffer
        PCODE RegContents;
        if ((opcode & 0xFF000000) == 0x58000000) //Actual LDR literal
        {
            RegContents = GetMem(ip);
        }
        else
        {
            RegContents = ip;
        ...
        LOG((LF_CORDB, LL_INFO100000, "Arm64Walker::Simulate opcode: %x  to update Reg X%d, as %p \n", opcode, RegNum, GetReg(context, RegNum)));
    }
    //else  Just execute the opcodes as IS
    //{
    //}
...
BOOL  NativeWalker::DecodePCRelativeBranchInst(PT_CONTEXT context, const PRD_TYPE& opcode, PCODE& offset, WALK_TYPE& walk)
{
    ...
    if ((opcode & 0xFF000010) == 0x54000000) // B.cond
    {
        ...
        {
            walk = WALK_BRANCH;
            offset = ((opcode >> 5) & 0x7FFFF)<<2; //imm19:00 -> 21 bits

            if (offset & 0x100000) //sign extension check for 21's bit
            {
                offset = offset | 0xFFFFFFFFFFE00000;
            }
    ...
    if ((opcode & 0xFE000000) == 0xB4000000) // CBNZ || CBZ
    {
        bool result = false;
        ...
        if (result)
        {
            walk = WALK_BRANCH;
            offset = ((opcode >> 5) & 0x7FFFF) << 2; //imm19:00 -> 21 bits
            if (offset & 0x100000) //sign extension check for 21'th bit
            {
                offset = offset | 0xFFFFFFFFFFE00000;
            }
    ...
    if ((opcode & 0x7E000000) == 0xB6000000)    // TBNZ || TBZ
    {
        bool result = false;
...
BOOL  NativeWalker::DecodeCallInst(PT_CONTEXT context, const PRD_TYPE& opcode, int& RegNum, WALK_TYPE& walk)
{
    if ((opcode & 0xFF9FFC1F) == 0xD61F0000) //Unconditional Branch (register) instructions
    {
        ...
        switch (op)
        {
        case 0b00: LOG((LF_CORDB, LL_INFO100000, "Arm64Walker::Decoded opcode: %x to BR X%d\n", opcode, RegNum));
            walk = WALK_BRANCH;
            break;
        case 0b01: LOG((LF_CORDB, LL_INFO100000, "Arm64Walker::Decoded opcode: %x to BLR X%d\n", opcode, RegNum));
            walk = WALK_CALL;
            break;
        case 0b10: LOG((LF_CORDB, LL_INFO100000, "Arm64Walker::Decoded opcode: %x to Ret X%d\n", opcode, RegNum));
            walk = WALK_RETURN;
            break;
        default: _ASSERTE(!("Arm64Walker::Decoded Unknown opcode"));
        }
